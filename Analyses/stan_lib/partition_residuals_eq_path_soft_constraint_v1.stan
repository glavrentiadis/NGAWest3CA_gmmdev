...
model {
  ...
  //------------------------------------
  // //scaled by lambda
  // SDdeltaBprime  ~ normal(tau0, 1/lambda);
  // SDdeltaBPprime ~ normal(tauP, 1/lambda);
  // SDdeltaSprime  ~ normal(phiS, 1/lambda);
  //scaled by the number of samples
  rep_array(calc_emp_std(deltaBprime,  0), N) ~ normal(tau0, 1/lambda);
  rep_array(calc_emp_std(deltaBPprime, 0), N) ~ normal(tauP, 1/lambda);
  rep_array(calc_emp_std(deltaSprime,  0), N) ~ normal(phiS, 1/lambda);
  rep_array(calc_emp_std(deltaWSprime, 0), N) ~ normal(phi0, 1/lambda);
  // //scaled by the number of samples
  // rep_array(calc_emp_std(deltaBprime,  0), NEQ)  ~ normal(tau0, 1/lambda);
  // rep_array(calc_emp_std(deltaBPprime, 0), NEQ)  ~ normal(tauP, 1/lambda);
  // rep_array(calc_emp_std(deltaSprime,  0), NSTA) ~ normal(phiS, 1/lambda);
  // rep_array(calc_emp_std(deltaWSprime, 0), N)    ~ normal(phi0, 1/lambda);
}
