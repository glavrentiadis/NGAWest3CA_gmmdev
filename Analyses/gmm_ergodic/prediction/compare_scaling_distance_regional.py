...
### ======================================
#initialize arrays
f_med = np.full((len(rrup_array), len(regions)), np.nan)
tau0  = np.full((len(rrup_array), len(regions)), np.nan)
tauP  = np.full((len(rrup_array), len(regions)), np.nan)
phiS  = np.full((len(rrup_array), len(regions)), np.nan)
phi0  = np.full((len(rrup_array), len(regions)), np.nan)
...
fname_fig = 'gmm_eas_cmp_dist_scaling_f_%.4fhz_M_%.1f_vs30_%.1fmsec_regional'%(freq,mag,vs30)
...
for k, reg in enumerate(regions):
    ...
    #new gmm
    hl_new  = ax.loglog(rrup_array, np.exp(f_med[:,k]), '-',  linewidth=lw, color=clr, label=lb, zorder=zo)
#ba19
