...
### ======================================
#initialize arrays
f_med = np.full(len(mag_array), np.nan)
tau0  = np.full(len(mag_array), np.nan)
tauP  = np.full(len(mag_array), np.nan)
phiS  = np.full(len(mag_array), np.nan)
phi0  = np.full(len(mag_array), np.nan)
...
fname_fig = 'gmm_eas_cmp_mag_scaling_f_%.4fhz_M_%.1f_vs30_%.1fmsec'%(freq,mag,vs30)
fig, ax = plt.subplots(figsize = (20,20), nrows=1, ncols=1)
#plot new and BA19 gmms
hl_new  = ax.semilogy(mag_array, np.exp(f_med),      '-',  linewidth=3., color='black', label='New GMM')
