...
#initialize arrays
f_med = np.full(n_f, np.nan)
tau0  = np.full(n_f, np.nan)
tauP  = np.full(n_f, np.nan)
phiS  = np.full(n_f, np.nan)
phi0  = np.full(n_f, np.nan)

#compute median ground moton
for j, (f, k_f) in enumerate(zip(freq_list, df_coeff_freqs)):
    #gmm regions
    regions = df_coeff_freqs[k_f].index.to_list()
    # regions = df_coeff_freqs[k_f].index[~np.isin(df_coeff_freqs[k_f].index, 'GLOBAL')].to_list()
   
    #linear coefficients
    c1    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c1'],   flag_reg, reg=regions)
    c2    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c2'],   flag_reg, reg=regions)
    c3    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c3'],   flag_reg, reg=regions)
    c4    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c4'],   flag_reg, reg=regions)
    c7    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c7'],   flag_reg, reg=regions)
    c8    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c8'],   flag_reg, reg=regions)
    c9    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c9'],   flag_reg, reg=regions)
    c10a  = parse_coeff(df_coeff_freqs[k_f].loc[:,'c10a'], flag_reg, reg=regions)
    c10b  = parse_coeff(df_coeff_freqs[k_f].loc[:,'c10b'], flag_reg, reg=regions)
    c11   = parse_coeff(df_coeff_freqs[k_f].loc[:,'c11'],  flag_reg, reg=regions)
    c13   = parse_coeff(df_coeff_freqs[k_f].loc[:,'c13'],  flag_reg, reg=regions)
 
    #non-linear coefficients
    c5    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c5'],   flag_reg, reg=regions)
    c6    = parse_coeff(df_coeff_freqs[k_f].loc[:,'c6'],   flag_reg, reg=regions)
    chm   = parse_coeff(df_coeff_freqs[k_f].loc[:,'chm'],  flag_reg, reg=regions)
    cn    = parse_coeff(df_coeff_freqs[k_f].loc[:,'cn'],   flag_reg, reg=regions)
    cmag  = parse_coeff(df_coeff_freqs[k_f].loc[:,'cmag'], flag_reg, reg=regions)

    #aleatory variability
    s1    = parse_coeff(df_coeff_freqs[k_f].loc[:,'s1'],   flag_reg, reg=regions)
    s2    = parse_coeff(df_coeff_freqs[k_f].loc[:,'s2'],   flag_reg, reg=regions)
    s3    = parse_coeff(df_coeff_freqs[k_f].loc[:,'s3'],   flag_reg, reg=regions)
    s4    = parse_coeff(df_coeff_freqs[k_f].loc[:,'s4'],   flag_reg, reg=regions)
    s5    = parse_coeff(df_coeff_freqs[k_f].loc[:,'s5'],   flag_reg, reg=regions)
    s6    = parse_coeff(df_coeff_freqs[k_f].loc[:,'s6'],   flag_reg, reg=regions)
    #aleatory variability (mag break)
    s1mag = parse_coeff(df_coeff_freqs[k_f].loc[:,'s1mag'], flag_reg, reg=regions)
    s2mag = parse_coeff(df_coeff_freqs[k_f].loc[:,'s2mag'], flag_reg, reg=regions)
    s5mag = parse_coeff(df_coeff_freqs[k_f].loc[:,'s5mag'], flag_reg, reg=regions)
    s6mag = parse_coeff(df_coeff_freqs[k_f].loc[:,'s6mag'], flag_reg, reg=regions)
...
fname_fig = 'gmm_eas_cmp_shape_M_%.1f_rrup_%.1fkm_vs30_%.1fmsec'%(mag,rrup,vs30)
fig, ax = plt.subplots(figsize = (20,20), nrows=1, ncols=1)
#plot new and BA19 gmms
hl_new  = ax.loglog(freq_list, np.exp(f_med), '-', linewidth=3., label='New GMM', color='black')
hl_ba19 = ax.loglog(gmm_ba19.freqs, gmm_ba19.eas, '--', linewidth=2., label='BA19', color='black')
#edit properties
